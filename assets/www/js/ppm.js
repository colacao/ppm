...
Date.prototype.toDateString = function(split) {
	split = split || "_";
	var tempArr = [this.getFullYear(), this.getMonth() + 1, this.getDate()];
	return tempArr.join(split);
}
Date.prototype.toDate2String = function(split) {
	split = split || "_";
	var tempArr = [this.getMonth() + 1, this.getDate().toString().pad(2)];
	return tempArr.join(split);
}
Date.prototype.toDateTimeString = function(split) {
	split = split || "_";
	var tempArr = [this.getHours()];
	return tempArr.join(split);
}
Date.prototype.toDateTime2String = function(split) {
	split = split || "_";
	var tempArr = [this.getFullYear(), this.getMonth() + 1, this.getDate(), this.getHours()];
	return tempArr.join(split);
}
$(function() {
...
$(function(){
	...
	var NOW = new Date();
var xData = {}
function getArr(data,key){
	var comps = {};
	$("#chkComp input").each(function(i,item){
		if(item.checked){
			comps[item.value] = item.value;
		}
	});
	...
	for(var i = 0;i<data.length;i++){
		var ttype = data[i].Company ? {
			"其它": "其它",
			"U": "联通",
			"T": "电信",
			"M": "移动"
		}[data[i].Company] : "其它";
		if(ttype == key && (ttype in comps)){
		
			var tempDateString  = new Date(data[i].Date).toDate2String('/');
			if(!xData[tempDateString]){
				xData[tempDateString] = tempDateString
			};
			arr.push(parseInt(data[i][LAST_TAB]*1));
		}
...
function abc(type) {
	$('#one').html("");
	var el = $('#selectmenu24');
	var eldata = [el.data('begin'),el.data('end')];
	var begin = eldata[0];
	var end = eldata[1];
	var pageid = $('#searchinput1').val().split(' ')[1];
	var params = {

		type:$('#selectmenu2').val(),
		begin: begin ? begin : new Date(new Date().setDate(-30)).toDateString("-"),
		end:end ?end: new Date().toDateString("-"),
		datetype: "DAY",
		pageid:pageid ? pageid : "100101",
		city: null
	};
	// $('#one').html('<img src="images/demo.png"  style="width:100%;height:100%" />');
	// return ;
	LAST_TAB = params.type;
	
	// var url = "./data/{$begin}/{$end}/{$datetype}/{$city}/NULL/{$pageid}/{$type}/Company".replaceWith(params);
	// $.ajax({
		// url: url,
		// dataType: "json",
		// success: function(data) {
			...
			NOW = data.date;
			if (!data.data.length) {
				$('#one').html("<div style='width:100%;height:100%;min-height: 400px;background-color:#fff;background-image:url(images/sorry.png);text-align:center;background-repeat:no-repeat;background-position: center;'></div>");
				return;
			}
			var seriesOptions  = [];
			var  names = {};
			...
			for(var i=0;i<data.data.length;i++){
				var ttype = data.data[i].Company ? {
						"其它": "其它",
						"U": "联通",
						"T": "电信",
						"M": "移动"
				}[data.data[i].Company] : "其它";
			...
			for(var b in names){
				seriesCounter++;
					//debugger;
				var arr = getArr(data.data,b);
				//console.log(arr);
				if(arr.length){
					seriesOptions[seriesCounter] = {
						name: b,
						data: arr,
						visible :arr.length?true:false,
						marker : {
							enabled : true,
							radius : 3
						}
			...
			for(var i=0;i<seriesOptions.length;i++){
				if(seriesOptions[i]){
					_seriesOptions.push(seriesOptions[i]);
				}
			}
