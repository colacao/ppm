;(function($) {
	...
	String.prototype.toInt = function () {
		return parseInt(this.replace(va, ""), 10);
	};
	var Calendar = function(opts) {
			var defaults = {
				...
				template: {
					head: '<div class="calendar_title"><a class="{$type}"></a>{$date}</div>',
					shead: '<div class="calendar_title"><a class="{$typel}"></a>{$date}<a class="{$typer}"></a></div>',
					body: '<table class="calendar_day"><tr><th class="weekend">日</th><th>一</th><th>二</th><th>三</th><th>四</th><th>五</th><th class="weekend">六</th></tr>{$dates}</table>',
					foot: '',
					day: '<td><a href="javascript:void(0);" {$id} {$Class} {$Style}>{$day}</a></td>',
	...
	Calendar.prototype = {
		...
		getClass: function(date, tempDateStr, input) {
			...
			var classList = [
			this.classNames.today, this.classNames.tomorrow, this.classNames.aftertomorrow];
		...
		getDates: function(date) {
			...
			for (var i = 1; i < 43; i++) {
				...
				if (i > beginDay && i <= nDays + beginDay) { //当前月 
					...
					returnValue.push(trE+trB+this.template.day.replaceWith({
						id: "id=" + tempId,
		...
		changeMonth: function(obj, op) {
			obj.date = new Date(
			obj.date.getFullYear(), obj.date.getMonth() + eval("(" + op + ")"), 1);
			this.render(this.date);
		}
	};
	...
	CalendarManage.prototype = {
		...
		setWeek: function(input) {
			...
			if (tips && tips.value().isDate()) {
				...
				if (txt && tips.offset().width >= 105) {
					$(input).css({
						"background-image": "url(http://pic.c-ctrip.com/cquery/" + txt + ".png)",
		...
		show: function(input) {
			...
			for (var i = 0; i < this.items.length; i++) {
				...
				if (date.length) {
					...
					ddate = new Date([now.getFullYear(), (now.getMonth() + i + 1), 1].join("/"))
				}

				this.items[i].input = this.input;
				var $input = $(this.input);
				_input.data('startDate', $input.data('startDate') || this.startDate);
				_input.data('endDate', $input.data('endDate') || this.endDate);
				_input.data('minDate', $input.data('minDate') || this.minDate);
				_input.data('maxDate', $input.data('maxDate') || this.maxDate);
				_input.data('showWeek', $input.data('showWeek') || this.showWeek);
				_input.data('rangeColor', $input.data('rangeColor') || this.rangeColor);
				_input.data('reference', $input.data('reference') || this.reference);
				_input.data('nextEl', $input.data('nextEl'));
				_input.data('prevEl', $input.data('prevEl'));
				_input.data('todayClass', $input.data('todayClass'));

				if (!input.value.isDate() && _input.data('reference')) {
					var reference = $($(this.input).data('reference')).value();
					if (reference.isDate()) {
						var _date = new Date(reference.replace(/-/g, '/'));
						var Year = _date.getFullYear();
						var Month = _date.getMonth() + i;
						ddate = new Date(Year, Month, 1);
					}
					//ddate = new Date(new Date(reference.replace(/-/g, '/')).setMonth(new Date(reference.replace(/-/g, '/')).getMonth() + i)) 
				};
		...
		override: function() {
			...
			for (var i = 0; i < this.items.length; i++) {
				...
				var date = this.date ? new Date(this.date.replace(/-/g, '/')).setMonth(new Date(this.date.replace(/-/g, '/')).getMonth() + i) : new Date(new Date().setMonth(new Date().getMonth() + i));
		 ...
		 */
		changeMonth: function(calobj, opt) {

			this.items.each(function(item, i) {
				this.items[i].date = new Date(
				this.items[i].date.getFullYear(), this.items[i].date.getMonth() + eval("(" + opt + ")"), 1);
				this.items[i].render(this.items[i].date);
			}.bind(this));
			this.input.onShow.call(this);
			this.setToday();
		...
		clickEvent: function(obj, tag) {
			...
			if (this.input && !$(tag).hasClass(obj.classNames.nothismonth) && tag.id) {
				...
				if (typeof this.listeners.onChange != "undefined") {
					 this.listeners.onChange.call(this, this.input, val, true);
				}
			}
		},
		/**
		...
		setTotalDay: function(day) {
			...
			this.beginDate.getFullYear(), this.beginDate.getMonth(), (this.beginDate.getDate() * 1 + day * 1));
	...
	var cm = function(objs, options) {
			...
			$.extend(defaults.listeners, options.listeners,true);
			$.extend(defaults, options.options,true);
			var $objs = $(objs);
		...
		$.fn.CalendarManage = function(options){
			...
			window.CalendarManage = new cm(this,defaults);
		};
	// var cls = {
		// name: 'calendar',
		// version: '3.0',
